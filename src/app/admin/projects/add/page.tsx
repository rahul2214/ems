...
export default function AdminAddProject() {
  const { projects, handleCreateProject } = useChronos();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!projCode.trim() || !projName.trim()) {
      alert("Project code and name are required.");
      return;
    }

    if (projects.some(p => p.code.toLowerCase() === projCode.trim().toLowerCase())) {
      alert("A project with this code already exists.");
      return;
    }
  ...
  return (
    <div className="content-view active-view" style={{ padding: '0px', maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
        ...
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 700, margin: '0 0 4px 0', color: 'var(--text-dark)' }}>Create New Project</h1>
          <p style={{ margin: 0, fontSize: '14px', color: 'var(--text-muted)' }}>Set up a new project to start tracking time and resources.</p>
        </div>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
            ...
            <div className="form-group">
              <label style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
                <Hash size={14} color="var(--primary)" /> Project Code <span style={{color: '#ef4444'}}>*</span>
              </label>
              <input
                type="text"
                placeholder="e.g. ABM006"
                value={projCode}
...
// Hash icon component since it wasn't imported from lucide-react in the old file
