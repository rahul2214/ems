...
export default function InternTimesheets() {
  ...
  const MANAGER_OPTIONS = selectedProjectObj
    ? (selectedProjectObj.members || [])
      .filter(m => {
        const globalEmp = employees.find(e => e.id === m.employee_id);
        return globalEmp?.role === 'manager' && m.profiles?.name;
      })
      .map(m => ({ id: m.employee_id, name: m.profiles!.name }))
    : [];
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
          ...
          <div className="form-group">
            ...
            <select
              ...
              required
            >
              <option value="">{MANAGER_OPTIONS.length === 0 && tsMasterCode ? "No Managers Assigned" : "Select Manager"}</option>
