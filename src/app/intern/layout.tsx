...
export default function InternLayout({ children }: { children: React.ReactNode }) {
  ...
  const getPageTitle = () => {
    if (pathname.includes('/intern/dashboard')) return 'Dashboard Overview';
    if (pathname.includes('/intern/timesheets')) return 'Timesheet Log & Submissions';
    if (pathname.includes('/intern/profile')) return 'Intern Personal Profile';
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          <Link
            ...
            className={`nav-item ${pathname === '/intern/dashboard' ? 'active' : ''}`}
            onClick={() => setSidebarOpen(false)}
          >
            <LayoutDashboard /> <span>Dashboard</span>
          </Link>
          <Link
            href="/intern/timesheets"
            className={`nav-item ${pathname === '/intern/timesheets' ? 'active' : ''}`}
            onClick={() => setSidebarOpen(false)}
          >
          ...
          <Link
            href="/intern/profile"
            className={`nav-item ${pathname === '/intern/profile' ? 'active' : ''}`}
            onClick={() => setSidebarOpen(false)}
          >
