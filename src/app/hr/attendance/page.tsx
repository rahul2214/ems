...
export default function HRAttendance() {
  const { timesheets, employees, getInitials } = useChronos();
  ...
  // Filter logic
  const filteredLogs = timesheets.filter(ts => {
    const emp = getEmployee(ts.employee_id);
    if (!emp) return false;
  ...
  const sortedLogs = [...filteredLogs].sort((a, b) => new Date(b.week_start_date).getTime() - new Date(a.week_start_date).getTime());
  ...
  const formatClockTime = (timeIso: string) => {
    ...
    return timeIso; // fallback if it's already HH:MM format
  };
